...
import com.codeberg.monsieurdoceo.filizer.utilities.FileChecker;
import java.io.File;
...
public final class FileCreator {
    ...
    public static File createFile(final Path path, final String name) {
        ...
        try {
            ...
            if (!FileChecker.fileExists(filePath)) Files.createFile(filePath);

            return filePath.toFile();
        } catch (IOException e) {
            throw new IllegalStateException(
                "[Filizer] Critical I/O error for " + name,
                e
            );
        }
    }
